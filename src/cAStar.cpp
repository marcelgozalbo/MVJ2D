...
void cAStar::NextCell(int *cx,int *cy)
{
	*cx=ReadPathX(1,idxStep);
	*cy=ReadPathY(1,idxStep);
	idxStep++;
}
...
//-----------------------------------------------------------------------------
int cAStar::FindPath (int pathfinderID,int startingX, int startingY,
...
//	Repeat the following until the new item in slot #1 sinks to its proper spot in the heap.
	...
	while (1);//!KeyDown(27));//reorder the binary heap
...
void cAStar::ReadPath(int pathfinderID,int currentX,int currentY,
			  int pixelsPerFrame)
{
/*
